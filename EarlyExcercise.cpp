...
EarlyExcercise::EarlyExcercise(const Simulation& theSimulation, const Vanilla& theVanilla)
{
...
vector<double> EarlyExcercise::Results() const
{
	...
	for(int time=PathDependents[0].size()-2;time>0;time--)//for every time instant
	{
				...
				//if(IndicatorOfAsian<1e-5) //No Asian
				//{
				...
				//Fitsvd myfit(Xnr,Ynr,SIC,cubicfit,TOL);
					//myfit.fit();
					//A=myfit.a;//The regression coefficients
				//}else
				//{
					//Fitsvd myfit(XPathDependent,Ynr,SIC,cubicfit1,TOL);
					//myfit.fit();
					//A=myfit.a;//The regression coefficients
				//}
					   ...
					   for(int i=0;i<Xnr.size();i++)
					   {
						   ...
						   if(IndicatorOfAsian<1e-5)//if path independent
						    expectedvalue=A[0]+A[1]*PathDependents[MoneynessIndex[i]][time]+A[2]*PathDependents[MoneynessIndex[i]][time]*PathDependents[MoneynessIndex[i]][time];
						   else
						   {
 ...
 Excercise* EarlyExcercise::clone() const
{
	return new EarlyExcercise(*this);
 }