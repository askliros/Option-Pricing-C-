...
vector<double> EarlyExcerciseRateVol::Results() const
{
	...
	int sumfwdtimes=0;
	for(int i=0;i<StockPaths.size();i++)
	{
	...
	for(int time=StockPaths[0].size()-2;time>0;time--)//for every time instant
	{
					...
					for(int pathindex=0;pathindex<StockPaths.size();pathindex++)//for every path
					{				
						if(theVanillaPtr->VanillaPayoff(StockPaths[pathindex][time])>0)
						{
							...
							for(int ftime=time+1;ftime<StockPaths[0].size();ftime++)
							{
								sumfwd=0;
				...
				cout<<StockPaths[MoneynessIndex[0]][time];
				    ...
				    A=theregression.regressratevolstoc(XRatesVolStock,Ynr);
					   ...
					   for(int i=0;i<Xnr.size();i++)
					   {
						   ...
						   {
							   for(int forwardtime=time+1;forwardtime<StockPaths[0].size();forwardtime++)
								{
	...
	for(int i=0;i<StockPaths.size();i++)
	{
		for(int j=1;j<StockPaths[0].size();j++)
		{
			summ+=StockPaths[i][j];
